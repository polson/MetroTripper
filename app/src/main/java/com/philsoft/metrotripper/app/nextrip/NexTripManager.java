...
public class NexTripManager extends BaseFragment {
	...
	public void getTripsRepeating(long stopId, int interval) {
		...
		if (currentRunnable != null && currentRunnable.stopId == stopId) {
			...
			log.d("Trips requested for same stop: " + stopId + ", returning existing trip data");
			notifyLoadComplete(Lists.newArrayList(trips));
			return;
		}

		// Different stop selected, cancel any existing runnables
		log.d("Started updating trips for stopId: " + stopId);
		stopUpdatingTrips();
	...
	public void stopUpdatingTrips() {
		if (currentRunnable != null) {
			log.d("Stopped updating trips for stopId: " + currentRunnable.stopId);
	...
	public void pauseUpdatingTrips() {
		if (currentRunnable != null) {
			log.d("Paused updating trips for stopId: " + currentRunnable.stopId);
			handler.removeCallbacks(currentRunnable);
		}
	}
	...
	public void resumeUpdatingTrips() {
		if (currentRunnable != null) {
			log.d("Resumed updating trips for stopId: " + currentRunnable.stopId);
			currentRunnable.run();
		}
	}
	...
	private void getTrips(final long stopId) {
		log.d("Getting trips for stopId: " + stopId);
		String url = Urls.getNexTripUrl(stopId);
		StringRequest request = new StringRequest(Request.Method.GET, url, new Response.Listener<String>() {
			@Override
			public void onResponse(String response) {
				try {
					log.d("Loaded trips for stopId: " + stopId);
